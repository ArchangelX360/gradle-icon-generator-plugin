...
A package cannot be deduced from the filepath of a Java source, it is a good practise to make it follow the directory
structure under `src/main/java`, but it could still be a valid Java source otherwise.

Because the required name generation is relative to the package, in `<fully_qualified_name_of_enclosing_class_of_the_field/slashed/instead/of/dotted>/<icon_field_name>.png`
...
Another example would be if we want to support multiple top-level classes in source files (which we do not for simplicity).
...
This way, we can fulfill the cleanup of deleted icons and the addition/update of added/changed fields.
